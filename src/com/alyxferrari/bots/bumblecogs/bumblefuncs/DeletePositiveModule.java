...
public class DeletePositiveModule implements BotModule {
	public boolean execute(MessageReceivedEvent mevent) {
		if (mevent.getMessage().getContentRaw().startsWith("bb;delpos")) {
				...
				if (parts.length > 1) {
					...
					if (BumbleBot.pos.remove(toAdd)) {
						...
						try {
							...
							ex.printStackTrace();
							mevent.getChannel().sendMessage(BumbleBot.getEmbed(null, "Failed to remove from positive phrase list.", Color.red).build()).queue();
						}
						mevent.getChannel().sendMessage(BumbleBot.getEmbed(null, "Removed from positive phrase list.", Color.green).build()).queue();
					} else {
