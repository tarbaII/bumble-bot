...
public class AddNegativeModule implements BotModule {
		...
		if (mevent.getMessage().getContentRaw().startsWith("bb;addneg")) {
			if (mevent.getMember().hasPermission(Permission.ADMINISTRATOR) || mevent.getMember().getId().equals("322896756713062400")) {
				...
				if (parts.length > 1) {
					...
					BumbleBot.neg.add(toAdd);
					try {
						PrintWriter writer = new PrintWriter(new FileOutputStream("bumbleneg.txt"));
						writer.println(toAdd);
						writer.flush();
						writer.close();
						mevent.getChannel().sendMessage(BumbleBot.getEmbed(null, "Added to negative phrase list.", Color.green).build()).queue();
					} catch (FileNotFoundException ex) {
						ex.printStackTrace();
						mevent.getChannel().sendMessage(BumbleBot.getEmbed(null, "Failed to add to negative phrase list.", Color.red).build()).queue();
					}
				}
			} else {
